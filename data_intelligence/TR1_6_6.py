...
# Load existing PSO engine — DO NOT MODIFY query_engine or TR1_6_6
...
# ============================================================================
# VECTOR DB PATHS — Update these to match your deployment directory
# ============================================================================
...
# ============================================================================
# CONFIDENCE LOGIC — Dynamic, no more hardcoded "High"
# ============================================================================
def compute_confidence(rag_loaded: bool, se_improvement_pct: float, pso_converged_early: bool) -> str:
    """
    Returns a realistic confidence string based on system state.
    Never returns High confidence when RAG is missing or PSO stalled.
    """
    if not rag_loaded and pso_converged_early:
        return (
            "LOW — Plant manual (RAG) unavailable AND optimizer converged early.\n"
            "   Action: Verify recommendations with shift engineer before applying."
        )
    if not rag_loaded:
        return (
            "MEDIUM — Based on mathematical PSO optimization only.\n"
            "   Plant manual knowledge not loaded. Cross-check with domain expert."
        )
    if pso_converged_early:
        return (
            "MEDIUM — RAG loaded but PSO swarm converged early (search may be incomplete).\n"
            "   Consider widening parameter bounds for next run."
        )
    if se_improvement_pct < 0.5:
        return (
            "MEDIUM — Improvement is marginal (<0.5%). Current state is near-optimal.\n"
            "   Monitor and re-run after any process disturbance."
        )
    return (
        "HIGH — PSO converged with meaningful improvement. "
        "Recommendations aligned with plant manual knowledge (RAG)."
    )


# ============================================================================
# REPORT FORMATTER — Operator-ready output
# ============================================================================
def format_parameter_table(changes_list: list) -> str:
    """
    Format all parameter changes as a clean aligned table for the LLM prompt.
    Each item in changes_list is a dict: {param, current, optimized, direction, pct_change}
    """
    if not changes_list:
        return "  No parameter changes recommended. Current state is within optimal bounds."

    lines = []
    lines.append(f"  {'Parameter':<48} {'Current':>10} {'Optimized':>10} {'Change':>10}")
    lines.append("  " + "-" * 82)
    for c in changes_list:
        arrow = "▲" if c["direction"] == "increased" else "▼"
        lines.append(
            f"  {arrow} {c['param']:<47} {c['current']:>10.3f} {c['optimized']:>10.3f} "
            f"  {c['pct_change']:>+.1f}%"
        )
    return "\n".join(lines)
...
def print_final_report(
    ...
    achieved_se: float,
    target_se: float,
    se_improvement_pct: float,
    lp_steam_reduction: float,
    changes_list: list,
    rag_loaded: bool,
    pso_converged_early: bool,
    llm_explanation: str,
    confidence: str
):
    """Print a structured, operator-ready report to console."""
    W = 70
    print("\n" + "═" * W)
    print("  PRESCRIPTIVE AI REPORT — EVAPORATION PLANT".center(W))
    print("═" * W)

    # ── KPI Summary ──
    print(f"\n  {'Steam Economy (Baseline):':<40} {baseline_se:.4f}")
    print(f"  {'Steam Economy (Optimized):':<40} {achieved_se:.4f}  (+{se_improvement_pct:.2f}%)")
    print(f"  {'Steam Economy (Target):':<40} {target_se:.4f}", end="")
    if achieved_se < target_se:
        print(f"  ⚠ Not reached — max safe step applied")
    else:
        print(f"  ✓ Target achieved")

    if lp_steam_reduction > 0:
        print(f"  {'LP Steam Reduction (est.):':<40} -{lp_steam_reduction:.3f} TPH")

    # ── RAG / PSO Status ──
    print("\n" + "─" * W)
    rag_label = "✓ Plant manual loaded" if rag_loaded else "⚠ Plant manual NOT loaded (RAG failed)"
    pso_label = "⚠ PSO converged early — search may be incomplete" if pso_converged_early else "✓ PSO converged normally"
    print(f"  RAG STATUS : {rag_label}")
    print(f"  PSO STATUS : {pso_label}")

    # ── Parameter Changes ──
    print("\n" + "─" * W)
    print("  RECOMMENDED PARAMETER CHANGES:")
    print(format_parameter_table(changes_list))

    # ── LLM Explanation ──
    print("\n" + "─" * W)
    print("  PROCESS EXPLANATION (AI):")
...
class IndustryAgent:
    def __init__(self):
        ...
        try:
            ...
            print("[LLM_AGENT] ✓ RAG loaded successfully.")
        except Exception as e:
            print(f"[WARN] RAG load failed: {e}")
            print("[WARN] Continuing WITHOUT plant manual knowledge.")
            self.rag_loaded = False

    # ─────────────────────────────────────────────────────────────────────────
    def get_plant_context(self, parameters_changed: list) -> str:
        """Query FAISS for manual excerpts relevant to changed parameters."""
        if not self.rag_loaded:
            return ""   # Handled explicitly in prompt — not silently ignored

        query  = "Effect on steam economy when adjusting: " + ", ".join(parameters_changed)
        vector = self.embedder.encode([query]).astype("float32")
        D, I   = self.index.search(vector, k=3)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    def _detect_early_convergence(self, best_df) -> bool:
        """
        Check if PSO converged early (same SE across multiple top results).
        If all top-3 rows have the same predicted SE, swarm likely stalled.
        """
        if len(best_df) < 2:
            return False
        top_se_values = best_df["Predicted_Steam_Economy"].head(3).round(4).values
        return len(set(top_se_values)) == 1

    # ─────────────────────────────────────────────────────────────────────────
    def run_pipeline(self, target_se: float = 4.4):
        print(f"\n[LLM_AGENT] Running End-To-End Pipeline for Target SE: {target_se}")

        # ── Current plant state (replace with live PI pull in production) ──
        current_state = {
        ...
        print("\n--- INITIATING PSO OPTIMIZER ---")
        best_df, baseline_se = optimize_steam_economy(
            current_state, target_se=target_se, n_particles=50, n_iterations=30
        )

        if best_df.empty:
            print("[LLM_AGENT] PSO could not optimize. Exiting.")
            return

        best_scenario        = best_df.iloc[0]
        achieved_se          = best_scenario["Predicted_Steam_Economy"]
        se_improvement_pct   = ((achieved_se - baseline_se) / baseline_se) * 100
        pso_converged_early  = self._detect_early_convergence(best_df)

        # Estimate LP Steam reduction (Evaporation / SE)
        evap_rate = current_state.get('Total_Evaporation_Rate', 263.07)
        current_lp_steam   = evap_rate / baseline_se if baseline_se > 0 else 0
        predicted_lp_steam = evap_rate / achieved_se  if achieved_se  > 0 else 0
        lp_steam_reduction = current_lp_steam - predicted_lp_steam

        # ── Step 2: Build complete parameter change list ──
        changes_list  = []   # structured — for report formatter
        changes_strs  = []   # text — for LLM prompt
        changed_params = []  # names — for RAG query

        for p in CONTROLLABLE_PARAMETERS:
            if p not in best_scenario:
                continue
            curr    = current_state.get(p, None)
            ...
            delta     = new_val - curr
            pct_chg   = (delta / curr * 100) if curr != 0 else 0
            direction = "increased" if delta > 0 else "decreased"

            # Include ALL changes, even small ones (threshold 0.001)
            if abs(delta) > 0.001:
                changes_list.append({
                    "param":     p,
                    "current":   curr,
                ...
                changes_strs.append(
                    f"  {p}: {curr:.3f} → {new_val:.3f} ({direction}, {pct_chg:+.1f}%)"
                )
                changed_params.append(p.split("(")[0].strip())

        if not changes_list:
            changes_strs = ["  No parameter changes recommended. Current state is near-optimal."]

        # ── Step 3: RAG Retrieval ──
        print("\n--- FETCHING DOMAIN KNOWLEDGE (RAG) ---")
        rag_context = self.get_plant_context(changed_params)

        # ── Step 4: Compute dynamic confidence ──
        confidence = compute_confidence(self.rag_loaded, se_improvement_pct, pso_converged_early)

        # ── Step 5: LLM for process explanation only ──
        print("\n--- GENERATING PROCESS EXPLANATION VIA LLM ---")

        # RAG context block shown to LLM
        if self.rag_loaded and rag_context:
            rag_block = f"PLANT MANUAL EXCERPTS:\n{rag_context}"
        else:
            rag_block = (
                "PLANT MANUAL EXCERPTS:\n"
                "[NOT AVAILABLE — faiss_index.bin could not be loaded. "
                "Do NOT reference any manual-based reasoning. "
                "Explain based on fundamental evaporation process physics only.]"
            )

        prompt_template = PromptTemplate(
            input_variables=["baseline_se", "achieved_se", "improvement_pct",
                             "changes", "rag_block", "rag_available"],
            template="""
You are a senior Process Control Engineer specializing in multi-effect evaporation plants.

OPTIMIZATION RESULT:
  Baseline Steam Economy : {baseline_se:.4f}
  Optimized Steam Economy: {achieved_se:.4f}  (improvement: {improvement_pct:+.2f}%)

PARAMETER CHANGES RECOMMENDED BY PSO:
{changes}

{rag_block}

STRICT INSTRUCTIONS:
1. Write ONLY the "Process Explanation" section — 3 bullet points maximum.
2. Each bullet must explain ONE specific parameter change from the list above and its direct effect on steam economy.
3. Only mention parameters that appear in the PARAMETER CHANGES list. Do NOT invent parameters.
4. If RAG is not available ({rag_available}), explain using evaporation thermodynamics and heat transfer principles only.
5. Do NOT write confidence, recommendations, or any section other than the explanation bullets.
6. Keep each bullet to 2 sentences maximum. Be precise and technical.

FORMAT:
• [Parameter name]: [What changed] → [Why this improves steam economy]
"""
        )
        ...
        llm_response = chain.invoke({
            ...
            "improvement_pct":  se_improvement_pct,
            "changes":          "\n".join(changes_strs),
            "rag_block":        rag_block,
            "rag_available":    "YES" if (self.rag_loaded and rag_context) else "NO"
        })

        llm_explanation = llm_response.content.strip()

        # ── Step 6: Print full structured report ──
        print_final_report(
            baseline_se         = baseline_se,
            achieved_se         = achieved_se,
            target_se           = target_se,
            se_improvement_pct  = se_improvement_pct,
            lp_steam_reduction  = lp_steam_reduction,
            changes_list        = changes_list,
            rag_loaded          = self.rag_loaded,
            pso_converged_early = pso_converged_early,
